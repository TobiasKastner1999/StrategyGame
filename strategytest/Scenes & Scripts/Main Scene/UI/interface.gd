...
signal rebake(node) # calls to rebake the navmesh
	...
	if Input.is_action_just_released("LeftClick"):
		...
		if interface_input_mode != 0:
			if building_placer_can_place and building_placer_location != Vector3.ZERO:
				...
				building_node.destroyed.connect(get_parent()._on_hq_destruction)
				rebake.emit(building_node)
...
func updateGamestateInfo():
	var state_text = ""
	state_text += "[b]Ferronite Crystals:[/b] " + str(Global.getResource(Global.player_faction, 0)) + "\n"
	state_text += "[b]Zenecium Crystals:[/b] " + str(Global.getResource(Global.player_faction, 1)) + "\n\n"
